...
function activate(context) {
    let doFormat = vscode.commands.registerCommand("formatall.doFormat", async function () {
        ...
        try {
            ...
            let cont = await vscode.window.showInformationMessage(
                `This process will iterate all of the files in the current workspace. Files will be opened and formatted in batches of ${configuration.quantity}. After a batch is completed, all files in the batch will be saved. All open files will then be closed before the next batch starts.

                Files matching these criteria will be formatted:
                Include: ${configuration.search.include}
                Exclude: ${configuration.search.exclude}

                There are ${folders.length} files that match this criteria.`,
                { modal: true },
                "Continue"
            );
            if (cont !== "Continue") return vscode.window.showInformationMessage("The formatting has been cancelled.");
            let i = 0;
    ...
    let doSave = vscode.commands.registerCommand("formatall.doSave", async function () {
        let configuration = vscode.workspace.getConfiguration("formatall");
    ...
    // Push Functions
    context.subscriptions.push(doFormat);
    context.subscriptions.push(doSave);
}
